...
public class InfiniteCraftGeminiClient {
	...
	@JSBody(params = { "apiKey", "model", "a", "b", "cb" }, script = ""
			...
			+ "fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',mode:'cors',headers:{'Content-Type':'application/json','Authorization':'Bearer '+apiKey},body:JSON.stringify({model:model,messages:[{role:'user',content:prompt}],temperature:0.7,max_completion_tokens:80,response_format:{type:'json_object'}})})"
			+ ".then(function(r){ if(!r.ok)return r.text().then(function(t){throw new Error('HTTP '+r.status+' '+String(t).slice(0,180));}); return r.json(); })"
			+ ".then(function(j){ var t=j&&j.choices&&j.choices[0]&&j.choices[0].message?j.choices[0].message.content:'{}'; var o; try{o=JSON.parse(t);}catch(e){var m=String(t).match(/\\{[\\s\\S]*\\}/);o=m?JSON.parse(m[0]):{};} o.name=cleanName(o.name); o.kind=o.kind==='item'?'item':'block'; finishFast(basePixels({name:o.name,kind:o.kind})); var art='pixel art 16x16 Minecraft '+o.kind+' icon of '+o.name+', created by combining '+a+' and '+b+', centered, readable silhouette, crisp square pixels, no text, no letters, transparent or simple background'; if(window.puter&&puter.ai&&puter.ai.txt2img){ return puter.ai.txt2img(art,{model:'black-forest-labs/flux-schnell'}).then(function(img){ var done=pixelate(img,o); done.generatedTexture=true; saveFinal(done); },function(){ return puter.ai.txt2img(art,{model:'gpt-image-1-mini',quality:'low'}).then(function(img){ var done=pixelate(img,o); done.generatedTexture=true; saveFinal(done); },function(){ var done=fallbackPixels(o); done.generatedTexture=true; saveFinal(done); }); }); } var done=fallbackPixels(o); done.generatedTexture=true; saveFinal(done); })"
